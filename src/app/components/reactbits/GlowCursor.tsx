...
// Coût GPU : le fragment shader boucle sur MAX_POINTS-1 pour chaque pixel.
// 32 segments suffisent largement pour la traînée et divisent ce coût par deux.
const MAX_POINTS = 32;
...
const GlowCursor = ({
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      // ⚡️ Repos : la traînée est complètement effacée et rien ne la rallume.
      // On saute la mise à jour de la chaîne ET le draw WebGL — le GPU ne fait
      // plus rien tant que le curseur ne bouge pas.
      const idleNow = now - lastInputTime;
      const fadingOut =
        !propsRef.current.enabled ||
        (propsRef.current.idleFade && (!pointerInside || idleNow > propsRef.current.idleTimeout));
      if (fade < 0.002 && fadingOut) {
        if (fade !== 0) {
          fade = 0;
