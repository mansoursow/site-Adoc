  ...
  useEffect(() => {
    ...
    const render = (now: number): void => {
      ...
      particles.forEach(particle => {
        ...
        if (Math.abs(dy) > maxDelta) maxDelta = Math.abs(dy);
      ...
      // ⚡️ Le texte est en place et plus rien ne le fait bouger : on coupe la
      // boucle d'animation. Elle repart au premier mouvement de souris.
      const settled = !gathering && maxDelta < 0.05 && (reducedMotion || idleDrift <= 0);
      if (settled) {
        animationFrame = null;
        return;
      }
